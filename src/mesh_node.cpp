...
void MeshNode::init_mesh()
{
    ...
    mesh.onDroppedConnection([](uint32_t nodeId)
        ...
        if (nodeId == node->bridgeId)
            ESP.deepSleep(10e6); });
    Task update_time(TASK_SECOND * 1, TASK_FOREVER, [this]()
                     { time_update(); });
    Task taskSendMessage(TASK_SECOND * 2, TASK_FOREVER, [this]()
                         { sendMessage(); });
...
void MeshNode::add_measurement(std::function<Measurements()> callable, unsigned long interval, long iterations)
{
    ...
    measure.set(TASK_SECOND * interval, TASK_FOREVER, [this, callable]()
